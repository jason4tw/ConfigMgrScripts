<#
.SYNOPSIS
    Cleans temporary files from the boot volume.
    
    .DESCRIPTION
        Cleans temporary files from the boot volume.
        
    .EXAMPLE
        .\Clean-BootDiskTempFiles.ps1
...
$win7subkeys = ("Active Setup Temp Folders","Downloaded Program Files","Internet Cache Files","Memory Dump Files","Old ChkDsk Files", `
                "Previous Installations","Recycle Bin","Service Pack Cleanup","Setup Log Files","System error memory dump files", `
...
$freeSpaceBefore = [math]::Floor(((Get-CIMInstance -Namespace root\cimv2 -Class Win32_LogicalDisk `
    -Filter "DeviceID='$env:SystemDrive'").FreeSpace) /1GB)

# Windows 7
if ((Get-CIMInstance -Namespace root\cimv2 -Class Win32_OperatingSystem).version -eq '6.1.7601')
{
    $subkeys = $win7subkeys
}
else
...
$volumeCaches = Get-Item -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches"

if($volumeCaches)
{
    foreach ($key in $subkeys)
    {
        Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches\$key" `
            -Name "StateFlags$sageSet" `
            -Value 2 `
            -Type DWord
    }

    Start-Process -FilePath "$env:SystemRoot\system32\cleanmgr.exe" `
    ...
    foreach ($key in $subkeys)
    {
        Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches\$key" `
            -Name "StateFlags$sageSet" `
    }

}

$freeSpaceAfter = [math]::Floor(((Get-CIMInstance -Namespace root\cimv2 -Class Win32_LogicalDisk `
    -Filter "DeviceID='$env:SystemDrive'").FreeSpace) /1GB)

$freeSpace = $freeSpaceBefore, $freeSpaceAfter
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBnoE6k/AMB9ILN
# r8EXIhZ8usqw/WnBAgjdzw9kOiuZP6CCGawwggSEMIIDbKADAgECAhBCGvKUCYQZ
...
# hvcNAQkEMSIEIGjHEQuSKeCjdCiGxgGoQ9irmTWOryTEydi7zYd/gBP5MA0GCSqG
# SIb3DQEBAQUABIIBALi73CiOgoDu8F+s8QlIHyvjUIbDfhvTKFhA35y2Ohl6E6zj
# J9BQf99nwYq+7lkgtU6EIuIktlv9WrULM2w7hbc1+y1ilQPMeeFpu55sPQEc+P8d
# YXIM+8pfDnLPctbVsBSBO15RogRqhLOOkgK8RuIUIVTkKaOH/Blf+FjjGxsP6Bvv
# ez1+mT+TwTVW4Ku7MdAPYZo/wo7MBCKDwT8d8sOv0yAvyhpnFi57e8Ge0UKPk+/w
# fsi5lxEZHk7OFtu5jpcBNQyO1/ujATEKdJNFz0xYvbBWczc8GComjp9snE8he9dO
# E1v/JICpRC67oPiFYSRvp2l2UbqYnW3NoXY9ThuhggIoMIICJAYJKoZIhvcNAQkG
...
# AQcBMBwGCSqGSIb3DQEJBTEPFw0xOTEyMDMwMDEwMjdaMCMGCSqGSIb3DQEJBDEW
# BBQhI4ZdDLIcDdiUl8oFzcPdrk+VWDANBgkqhkiG9w0BAQEFAASCAQBhYCHUG9lH
# 3hHzERotW8+gR2Tuc+P4hMz8+pxrN038ZHoctRec0FmePEU7b2+q57NeY/6L4Si4
# IYCNj5oVcXwbFovIrQ0Gp9zteFsQIjBxXabFmTmEauT+GL4us4ZjOg/hC00esvRk
# xntIhfJiIPJ0XLFWnENKP0x/DqM+abVfZ2FDgs+2Dr3CePuUAmSobhoPvDaluNtE
# kP6w1NTDHhWQb8+PES6FZroQZOvBQacSU9F7fuXtZw9Xa170jKj+NhMwN8mni2HE
# mQ48U+o35K6oHRPOX6d5Y+LdKUhU30RQHI1wOEe0eg1Us4YICQyxdd/m3nAWYy9t
# 77WPUx696KId
